...
// ---------------------------------------------------------------------------
// Injection types for gpu-share: pre-created wgpu-core objects
// Place these in OpState before creating the JsRuntime to bypass normal
// adapter/device enumeration and use our existing Vulkan objects instead.
// ---------------------------------------------------------------------------

/// Pre-registered adapter — `request_adapter` returns this instead of enumerating.
pub struct InjectedAdapterId(pub wgpu_core::id::AdapterId);

/// Pre-registered device+queue — `request_device` returns these instead of creating new ones.
pub struct InjectedDeviceIds {
    pub device_id: wgpu_core::id::DeviceId,
    pub queue_id: wgpu_core::id::QueueId,
}

/// Pre-registered texture — `importSharedTexture()` returns this as a GPUTexture.
/// Can be updated (replaced in OpState) on resize to point at the new texture.
pub struct InjectedTexture {
    pub texture_id: wgpu_core::id::TextureId,
    pub width: u32,
    pub height: u32,
    pub format: texture::GPUTextureFormat,
    pub usage: u32,
}
...
impl GPU {
  ...
  async fn request_adapter(
    ...
    // gpu-share: check for injected adapter first
    if let Some(injected) = state.try_borrow::<InjectedAdapterId>() {
      let id = injected.0;
      let instance = state.borrow::<Instance>();
      return Some(adapter::GPUAdapter {
