...
impl GPUDevice {
  ...
  // gpu-share: import the pre-registered shared surface texture
  #[cppgc]
  fn importSharedTexture(
    &self,
    state: &mut OpState,
  ) -> Result<GPUTexture, JsErrorBox> {
    let injected = state
      .try_borrow::<crate::InjectedTexture>()
      .ok_or_else(|| JsErrorBox::generic("No shared texture available (not injected)"))?;

    Ok(GPUTexture {
      instance: self.instance.clone(),
      error_handler: self.error_handler.clone(),
      id: injected.texture_id,
      device_id: self.id,
      queue_id: self.queue,
      default_view_id: Default::default(),
      label: "shared_surface".to_string(),
      ...
      mip_level_count: 1,
      sample_count: 1,
      dimension: super::texture::GPUTextureDimension::D2,
      format: injected.format.clone(),
      usage: injected.usage,
    })
  }
}
