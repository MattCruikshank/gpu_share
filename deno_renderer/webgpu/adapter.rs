...
impl GPUAdapter {
  ...
  fn request_device(
    ...
    // gpu-share: check for injected device/queue IDs
    let (device, queue) = if let Some(injected) = state.try_borrow::<crate::InjectedDeviceIds>() {
      (injected.device_id, injected.queue_id)
    } else {
      let features = self.instance.adapter_features(self.id);
      ...
      self.instance.adapter_request_device(
        self.id,
        &wgpu_descriptor,
        None,
        None,
